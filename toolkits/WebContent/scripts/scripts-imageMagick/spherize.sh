#!/bin/bash
# 
# Developed by Fred Weinhaus 6/20/2009 .......... revised 8/21/2012
# 
...
# greater than zero. The default=1. Values larger than one will make more 
# distortion and values less than one will make less distortion.
#
# -z zoom --- ZOOM is the zoom factor for the input image before warping it 
# onto the sphere. The default=1. Values larger than one will magnify the 
# input image on the sphere. Values less than one will minify the input image 
# on the sphere.
# 
	...
	while [ $# -gt 0 ]
	  ...
	  -h|-help)    # help information
			...
			-a)    # amp
				   ...
				   amptest=`echo "$amp <= 0" | bc`
				   [ $amptest -eq 1 ] && errMsg "--- AMPLIFICATION=$amp MUST BE A POSITIVE FLOATING POINT VALUE ---"
				   ;;
			-z)    # zoom
				   shift  # to get the next parameter
				   # test if parameter starts with minus sign 
				   ...
				   zoomtest=`echo "$zoom <= 0" | bc`
				   [ $zoomtest -eq 1 ] && errMsg "--- ZOOM=$zoom MUST BE A POSITIVE FLOATING POINT VALUE ---"
				   ;;
			-b)    # get bcolor
				   shift  # to get the next parameter
				   # test if parameter starts with minus sign 
...
trap "rm -f $tmpA1 $tmpA2 $tmpF1 $tmpF2 $tmpM1 $tmpM2 $tmpX1 $tmpX2 $tmpY1 $tmpY2; exit 0" 0
trap "rm -f $tmpA1 $tmpA2 $tmpF1 $tmpF2 $tmpM1 $tmpM2 $tmpX1 $tmpX2 $tmpY1 $tmpY2; exit 1" 1 2 3 15
...
elif [ "$bcolor" = "image" ]; then
	convert $tmpA1 $trimming $outfile
else
	convert \( $tmpA1 $tmpM1 \
